...
$rdf.Fetcher.prototype.requiresProxy = function(uri) {
	return uri.indexOf($rdf.Fetcher.homeServer) !== 0 || !uri.indexOf($rdf.Fetcher.crossSiteProxyTemplate) !== 0
}
...
//				console.log("not in correct promise! fetch (uri=" + uri + ")==(uri2=" + uri2 + ") is (with proxy verif) false");
		...
		if (sta == 'unrequested') {
			var proxyURI = (self.requiresProxy(uri))?$rdf.Fetcher.crossSiteProxy(uri):uri
