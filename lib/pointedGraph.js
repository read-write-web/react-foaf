...
$rdf.PointedGraph = function() {
    $rdf.PointedGraph = function(store, pointer, webGraphId, internalId){
        ...
        this.internalGraph = (internalId)?internalId:(webGraphId?$rdf.sym($rdf.Fetcher.crossSiteProxy(webGraphId.uri)):undefined);
    };
    ...
    $rdf.PointedGraph.prototype.getGraphTriples = function() {
        return this.store.statementsMatching(undefined, undefined, undefined, this.internalGraph, true)
    }

    $rdf.PointedGraph.prototype.isGraphHasTriples = function() {
        return this.getGraphTriples().length > 0; // TODO not optimized
    }

    $rdf.PointedGraph.prototype.isPointerExistHasSubject = function() {
        return this.store.statementsMatching(this.pointer, undefined, undefined, this.internalGraph, true).length > 0; // TODO not optimized
    }

    $rdf.PointedGraph.prototype.isPointerExistHasObject = function() {
        return this.store.statementsMatching(undefined, undefined, this.pointer, this.internalGraph, true).length > 0; // TODO not optimized
    }
    ...
    $rdf.PointedGraph.prototype.jumpNowOrLater = function(existenceCheck) {
        ...
        if (this.isLocalPointer()) return this;
            ...
            if ( this.pointer.value == "https://my-profile.eu/people/tim/card#me" ) {
                console.error("Will try to jump from: ",this.printSummary());
            }
            var docURL = $rdf.Stmpl.fragmentless(this.pointer.uri);
            var proxiedDoc = $rdf.sym($rdf.Fetcher.crossSiteProxy(docURL)); // TODO, do not force access though proxy
    ...
    $rdf.PointedGraph.prototype.jumpAsync = function(existenceCheck) {
        ...
        if (originalPG.isLocalPointer()) {
            ...
            return Q.fcall(function (){return originalPG })
        }
        else {
            var definitionalResource = originalPG.pointer.uri.split('#')[0];
            var fetcher = this.Fetcher();
            debug("notLocal:"+originalPG,originalPG)
            return fetcher.fetch(definitionalResource, originalPG.webGraph.uri )
                .then(function(pg) {
                    debug("isGraphHasTriples=",pg.isGraphHasTriples()," - isPointerExistHasSubject=",pg.isPointerExistHasSubject()," - isPointerExistHasObject=",pg.isPointerExistHasObject(),pg);
                    if ( pg.isGraphHasTriples() ) {
                        if ( !check || pg.isPointerExistHasSubject() || pg.isPointerExistHasObject() ) {
                            debug("Returning pg",pg);
                            //return pg;
                            // TODO !!!!!! it seems bugged to me, we should be able to return the pg dirrectly IMO (it doesn't work) so the fetcher may have a bug!
                            return $rdf.pointedGraph(pg.store, originalPG.pointer, pg.webGraph, pg.internalGraph)
                        } else {
                            warning("returning original PG because the jumped pointer doesn't seem to exist in the store",pg,originalPG);
                            return originalPG
                        }
                    } else {
                        warning("returning original PG because graph has no triples",originalPG);
                        return originalPG
                    }
                }
            )
