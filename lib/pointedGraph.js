...
$rdf.PointedGraph = function() {
	...
	$rdf.PointedGraph.prototype.jumpAsync = function(existenceCheck) {
		...
		if (this.isLocalPointer()) {
			...
			var definitionalResource = this.pointer.uri.split('#')[0];
			var fetcher = new $rdf.Fetcher(this.store)
...
//						+ " as object: "+pointerExistsAsObject())
	...
	$rdf.PointedGraph.prototype.jumpRel = function(relUri) {
		...
		var jumpedPGs = _.chain(localRemote.false ? localRemote.false : [])
			.map(function (pg) {
				var docURL = pg.pointer.uri.split('#')[0];
				var doc = $rdf.sym($rdf.Fetcher.crossSiteProxy(docURL))
				if (pg.store.statementsMatching(undefined, undefined, undefined, doc, false) > 0) {
					console.log("there is dta in " + doc + " so we make a new PG for it")
					var doc = $rdf.sym($rdf.Fetcher.crossSiteProxy(uri))
					return $rdf.pointedGraph(pg.store, pg.pointer, $rdf.sym(pg.docUrl), doc)
				} else {
					return pg
				}
				var res = localRemote.true.concat(jumpedPGs)
				return res;
			}).value()
