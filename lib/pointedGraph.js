                ...
                .then(function(pg) {
                    ...
                    if ( pg.isGraphHasTriples() ) {
                        if ( !check || pg.isPointerExistHasSubject() || pg.isPointerExistHasObject() ) {
                            ...
                            info("returning original PG because the jumped pointer doesn't seem to exist in the store",pg,originalPG);
                            return originalPG
                        }
                    } else {
                        info("returning original PG because graph has no triples",pg);
                        return originalPG
                    }
                }
            )
